...
$include "test_buffer/all_molecules_test_buffer.txt"

	J(A)				set of protein coding regions (transcripts)
$include "test_buffer/transcripts_test_buffer.txt"

	E(A)				set of proteins enzymes or polypeptides in the model
$include "test_buffer/enzymes_test_buffer.txt"

	P					set of promotors
$include "test_buffer/promoters_test_buffer.txt"

	Ed(E)				set of proteins which are desired to be the response signal to ligand signals
$include "test_buffer/desired_enzymes_test_buffer.txt"
 
	L1(A)				set of ligands
$include "test_buffer/Ligands_test_buffer.txt"

	Ld(L1)				set of ligands for which a response is desired
$include "test_buffer/desired_ligands_test_buffer.txt"

	T					set of terminators
$include "test_buffer/terminators_test_buffer.txt"

	tau					set of time values
$include "test_buffer/time_set_test_buffer.txt"
...
$include "test_buffer/promoter_strength_test_buffer.txt"

	eta(J)						efficiency of ribosome binding sites
$include "test_buffer/translation_efficiency_test_buffer.txt"

	Z(P)						normal state of promotor 1 of normally on 0 otherwise
$include "test_buffer/promoter_normal_state_test_buffer.txt"

	Zeta(E)						normal state of proteins 1 if normally active 0 otherwise
$include "test_buffer/protein_normal_state_test_buffer.txt"

	I(P,A)						promotor molecule interaction -1 if protein j represses promotor p 0 if no effect 1 if activation
$include "test_buffer/promoter_ligand_interactions_test_buffer.txt"
	
	B(E,A)						protein molecule interaction -1 if molecule A represses enzyme E 0 if no effect 1 if activation
$include "test_buffer/protein_ligand_interactions_test_buffer.txt"

	G(T)						strength of terminators
$include "test_buffer/terminator_strength_test_buffer.txt"

	R(E)						enzyme degradation rate
$include "test_buffer/protein_degradation_test_buffer.txt"
...
$include "test_buffer/expression_threshold_test_buffer.txt"

	lambda(Ld,Ed)			gives the values of W that should exist for desired proteins responses to desired ligands
$include "test_buffer/logic_table_test_buffer.txt"

	rho(J,E)					maps transcripts to enzymes
$include "test_buffer/trans_to_enzyme_test_buffer.txt"

	sigma(A,A)					determines if enzyme E is the same as enzyme E
$include "test_buffer/self_test_buffer.txt"
...
$include "test_buffer/promoter_leakiness_test_buffer.txt"
...
$include "test_buffer/promoter_ligand_strength_test_buffer.txt"
	
	Q(E,A) 						strength of ligand-protein interactions
$include "test_buffer/protein_ligand_strength_test_buffer.txt"
...
$include "test_buffer/desired_enzymes_test_buffer_vals.txt"

	Ld_val(L1)					set of ligands for which a response is desired value of 1 if a desired ligand zero otherwise
$include "test_buffer/desired_ligands_test_buffer_vals.txt"
...
FILE SIMPLECID / a_simple_EUGENECID_report_test_buffer.txt /;
PUT SIMPLECID;
SIMPLECID.pc = 2;
SIMPLECID.lw = 25;
...
FILE DETAILCID / a_detailed_EUGENECID_report_test_buffer.txt /;
PUT DETAILCID;
DETAILCID.pc = 2;
DETAILCID.lw = 25;
...
FILE DESIGNS / circuit_designs_file_test_buffer.txt /;
PUT DESIGNS;
DESIGNS.pc = 2;
DESIGNS.lw = 30;
...
FILE STATES / circuit_state_test_buffer.csv /;
PUT STATES;
STATES.pc = 5;
STATES.lw = 30;
STATES.pw = 32767;
...
FILE TABLES / circuit_logic_tables_test_buffer.txt /;
PUT TABLES;
TABLES.pc = 2;
TABLES.pw = 32767;
