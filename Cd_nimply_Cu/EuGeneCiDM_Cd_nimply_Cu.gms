...
$include "Cd_nimply_Cu/all_molecules_Cd_nimply_Cu.txt"

	J(A)				set of protein coding regions (transcripts)
$include "Cd_nimply_Cu/transcripts_Cd_nimply_Cu.txt"

	E(A)				set of proteins enzymes or polypeptides in the model
$include "Cd_nimply_Cu/enzymes_Cd_nimply_Cu.txt"

	P					set of promotors
$include "Cd_nimply_Cu/promoters_Cd_nimply_Cu.txt"

	Ed(E)				set of proteins which are desired to be the response signal to ligand signals
$include "Cd_nimply_Cu/desired_enzymes_Cd_nimply_Cu.txt"
 
	L1(A)				set of ligands
$include "Cd_nimply_Cu/Ligands_Cd_nimply_Cu.txt"

	Ld(L1)				set of ligands for which a response is desired
$include "Cd_nimply_Cu/desired_ligands_Cd_nimply_Cu.txt"

	T					set of terminators
$include "Cd_nimply_Cu/terminators_Cd_nimply_Cu.txt"

	tau					set of time values
$include "Cd_nimply_Cu/time_set_Cd_nimply_Cu.txt"
...
$include "Cd_nimply_Cu/promoter_strength_Cd_nimply_Cu.txt"

	eta(J)						efficiency of ribosome binding sites
$include "Cd_nimply_Cu/translation_efficiency_Cd_nimply_Cu.txt"

	Z(P)						normal state of promotor 1 of normally on 0 otherwise
$include "Cd_nimply_Cu/promoter_normal_state_Cd_nimply_Cu.txt"

	Zeta(E)						normal state of proteins 1 if normally active 0 otherwise
$include "Cd_nimply_Cu/protein_normal_state_Cd_nimply_Cu.txt"

	I(P,A)						promotor molecule interaction -1 if protein j represses promotor p 0 if no effect 1 if activation
$include "Cd_nimply_Cu/promoter_ligand_interactions_Cd_nimply_Cu.txt"
	
	B(E,A)						protein molecule interaction -1 if molecule A represses enzyme E 0 if no effect 1 if activation
$include "Cd_nimply_Cu/protein_ligand_interactions_Cd_nimply_Cu.txt"

	G(T)						strength of terminators
$include "Cd_nimply_Cu/terminator_strength_Cd_nimply_Cu.txt"

	R(E)						enzyme degradation rate
$include "Cd_nimply_Cu/protein_degradation_Cd_nimply_Cu.txt"
...
$include "Cd_nimply_Cu/expression_threshold_Cd_nimply_Cu.txt"

	lambda(Ld,Ld1,Ed)			gives the values of W that should exist for desired proteins responses to desired ligands
$include "Cd_nimply_Cu/logic_table_Cd_nimply_Cu.txt"

	rho(J,E)					maps transcripts to enzymes
$include "Cd_nimply_Cu/trans_to_enzyme_Cd_nimply_Cu.txt"

	sigma(A,A)					determines if enzyme E is the same as enzyme E
$include "Cd_nimply_Cu/self_Cd_nimply_Cu.txt"
...
$include "Cd_nimply_Cu/promoter_leakiness_Cd_nimply_Cu.txt"
...
$include "Cd_nimply_Cu/promoter_ligand_strength_Cd_nimply_Cu.txt"
	
	Q(E,A) 						strength of ligand-protein interactions
$include "Cd_nimply_Cu/protein_ligand_strength_Cd_nimply_Cu.txt"
...
$include "Cd_nimply_Cu/desired_enzymes_Cd_nimply_Cu_vals.txt"

	Ld_val(L1)					set of ligands for which a response is desired value of 1 if a desired ligand zero otherwise
$include "Cd_nimply_Cu/desired_ligands_Cd_nimply_Cu_vals.txt"
...
FILE SIMPLECID / a_simple_EUGENECID_report_Cd_nimply_Cu.txt /;
PUT SIMPLECID;
SIMPLECID.pc = 2;
SIMPLECID.lw = 25;
...
FILE DETAILCID / a_detailed_EUGENECID_report_Cd_nimply_Cu.txt /;
PUT DETAILCID;
DETAILCID.pc = 2;
DETAILCID.lw = 25;
...
FILE DESIGNS / circuit_designs_file_Cd_nimply_Cu.txt /;
PUT DESIGNS;
DESIGNS.pc = 2;
DESIGNS.lw = 30;
...
FILE STATES / circuit_state_Cd_nimply_Cu.csv /;
PUT STATES;
STATES.pc = 5;
STATES.lw = 30;
STATES.pw = 32767;
...
FILE TABLES / circuit_logic_tables_Cd_nimply_Cu.txt /;
PUT TABLES;
TABLES.pc = 2;
TABLES.pw = 32767;
