...
$include "test_adder/all_molecules_test_adder.txt"

	J(A)				set of protein coding regions (transcripts)
$include "test_adder/transcripts_test_adder.txt"

	E(A)				set of proteins enzymes or polypeptides in the model
$include "test_adder/enzymes_test_adder.txt"

	P					set of promotors
$include "test_adder/promoters_test_adder.txt"

	Ed(E)				set of proteins which are desired to be the response signal to ligand signals
$include "test_adder/desired_enzymes_test_adder.txt"
 
	L1(A)				set of ligands
$include "test_adder/Ligands_test_adder.txt"

	Ld(L1)				set of ligands for which a response is desired
$include "test_adder/desired_ligands_test_adder.txt"

	T					set of terminators
$include "test_adder/terminators_test_adder.txt"

	tau					set of time values
$include "test_adder/time_set_test_adder.txt"
...
$include "test_adder/promoter_strength_test_adder.txt"

	eta(J)						efficiency of ribosome binding sites
$include "test_adder/translation_efficiency_test_adder.txt"

	Z(P)						normal state of promotor 1 of normally on 0 otherwise
$include "test_adder/promoter_normal_state_test_adder.txt"

	Zeta(E)						normal state of proteins 1 if normally active 0 otherwise
$include "test_adder/protein_normal_state_test_adder.txt"

	I(P,A)						promotor molecule interaction -1 if protein j represses promotor p 0 if no effect 1 if activation
$include "test_adder/promoter_ligand_interactions_test_adder.txt"
	
	B(E,A)						protein molecule interaction -1 if molecule A represses enzyme E 0 if no effect 1 if activation
$include "test_adder/protein_ligand_interactions_test_adder.txt"

	G(T)						strength of terminators
$include "test_adder/terminator_strength_test_adder.txt"

	R(E)						enzyme degradation rate
$include "test_adder/protein_degradation_test_adder.txt"
...
$include "test_adder/expression_threshold_test_adder.txt"

	lambda(Ld,Ld1,Ld2,Ed)			gives the values of W that should exist for desired proteins responses to desired ligands
$include "test_adder/logic_table_test_adder.txt"

	rho(J,E)					maps transcripts to enzymes
$include "test_adder/trans_to_enzyme_test_adder.txt"

	sigma(A,A)					determines if enzyme E is the same as enzyme E
$include "test_adder/self_test_adder.txt"
...
$include "test_adder/promoter_leakiness_test_adder.txt"
	...
	ECTrans(E,L1,L2,L3) 			enzyme concentration transfered between time points
	
	RNATrans(J,T,L1,L2,L3)			RNA transcript level transfered between time points 
...
$include "test_adder/promoter_ligand_strength_test_adder.txt"
	
	Q(E,A) 						strength of ligand-protein interactions
$include "test_adder/protein_ligand_strength_test_adder.txt"
...
$include "test_adder/desired_enzymes_test_adder_vals.txt"

	Ld_val(L1)					set of ligands for which a response is desired value of 1 if a desired ligand zero otherwise
$include "test_adder/desired_ligands_test_adder_vals.txt"
...
ECTrans(E,L1,L2,L3) = 0;
RNATrans(J,T,L1,L2,L3) = 0;
...
VARIABLES
	...
	alpha(P,L1,L2,L3)				alpha determies net effect of all inhibition and activation effects on the given promotor
	gamma(E,L1,L2,L3)				gamma determies net effect of all inhibition and activation effects on the given protein

NONNEGATIVE VARIABLES

	/*these variables are shared by CID and CIS formulations*/
	C(E,L1,L2,L3)					proxy concentration of protein j subject to ligands
	phi(J,T,L1,L2,L3)				RNA level
	xi(P,J,T,L1,L2,L3)				value of deliberate transcription	
...
BINARY VARIABLES
	...
	C_plus(E,L1,L2,L3)				determines if the concentration activation threshold is achieved by comparison to Cjt
	W(E,L1,L2,L3)					determines if the protein is active based on inhibition activation transcription and translation
	alpha_plus(P,L1,L2,L3)			value of 1 if the promotor is active value of zero otherwise
	gamma_plus(E,L1,L2,L3)			value of 1 if the protein is active value of zero otherwise
	omega(E,L1,L2,L3)				value of 1 if enzyme protein or polypeptide E is produced 0 otherwise
	kappa(E,L1,L2,L3)				value of 1 is protein is active (e.g. not inhibited or is activated) 0 otherwise
...
EQUATIONS
	...
	act_threshold_0(E,Ld,Ld1,Ld2)		constraint used to trigger the activation threshold
	act_threshold_1(E,Ld,Ld1,Ld2)		constraint used to trigger the activation threshold
	prot_sum(E,Ld,Ld1,Ld2)				find the sum of transcript levels for each transcript C
	trans_sum(J,T,Ld,Ld1,Ld2)			find the sum of transcript levels for each transcript
	...
	findalpha(P,Ld,Ld1,Ld2)				find value of alpha 
	findalpha_sign0(P,Ld,Ld1,Ld2)		find sign of alpha
	findalpha_sign1(P,Ld,Ld1,Ld2)		find sign of alpha
	
	transcribed0(P,J,T,Ld,Ld1,Ld2)		Determines if j is transcribed by p in the current conditions
	transcribed1(P,J,T,Ld,Ld1,Ld2)		Determines if j is transcribed by p in the current conditions
	transcribed2(P,J,T,Ld,Ld1,Ld2)		Determines if j is transcribed by p in the current conditions
	
	produced0(E,Ld,Ld1,Ld2)				determines if a polypeptide is produced
	produced1(E,Ld,Ld1,Ld2)				determines if a polypeptide is produced
	
	findgamma(E,Ld,Ld1,Ld2)				Find gamma
	findgamma_sign0(E,Ld,Ld1,Ld2)		find value of gamma_plus
	findgamma_sign1(E,Ld,Ld1,Ld2)		find value of gamma_plus
	
	prot_could_act0(E,Ld,Ld1,Ld2)		determintes if protein could be active based on ligand inputs and other active protiens
	prot_could_act1(E,Ld,Ld1,Ld2)		determintes if protein could be active based on ligand inputs and other active protiens
	prot_could_act2(E,Ld,Ld1,Ld2)		determintes if protein could be active based on ligand inputs and other active protiens
	
	prot_is_act0(E,Ld,Ld1,Ld2)			determines if protein is active based on inhibition activation and concentration
	prot_is_act1(E,Ld,Ld1,Ld2)			determines if protein is active based on inhibition activation and concentration
	prot_is_act2(E,Ld,Ld1,Ld2)			determines if protein is active based on inhibition activation and concentration
	
	sat_lambda(Ld,Ld1,Ld2,Ed)			ensures solution satisfies the logic table
	...
	explicit_0(E)
	explicit_1(E,Ld,Ld1,Ld2)
	explicit_2(E,Ld,Ld1,Ld2)
	explicit_3(E,Ld,Ld1,Ld2)	
	explicit_4(E,Ld,Ld1,Ld2)	
	explicit_5(E,Ld,Ld1,Ld2)	
	
	/*simulation equations for EuGeneCiM*/
	obj_M								objective function
	prot_conc_M(E,Ld,Ld1,Ld2)			protein concentration
	trans_level_M(J,T,Ld,Ld1,Ld2)		find the transcript level for each transcript
	
	produced0_M(E,Ld,Ld1,Ld2)			determines if a polypeptide is produced
	produced1_M(E,Ld,Ld1,Ld2)			determines if a polypeptide is produced
	
	act_threshold_0_M(E,Ld,Ld1,Ld2)		constraint used to trigger the activation threshold
	act_threshold_1_M(E,Ld,Ld1,Ld2)		constraint used to trigger the activation threshold
...
*objective function
obj..									Z_D =e= sum(Ed, sum(Ld, sum(Ld1, sum(Ld2, C(Ed,Ld,Ld1,Ld2) * lambda(Ld,Ld1,Ld2,Ed) - C(Ed,Ld,Ld1,Ld2) * (1 - lambda(Ld,Ld1,Ld2,Ed))))));
...
findalpha(P,Ld,Ld1,Ld2)..				alpha(P,Ld,Ld1,Ld2) =e= Z(P) + sum(E, (W(E,Ld,Ld1,Ld2) * I(P,E) * H(P,E))) + I(P,Ld) * H(P,Ld) + I(P,Ld1) * H(P,Ld1)  + I(P,Ld2) * H(P,Ld2) - (I(P,Ld) * H(P,Ld) * sigma(Ld,Ld1)) - (I(P,Ld) * H(P,Ld) * sigma(Ld,Ld2)) - (I(P,Ld1) * H(P,Ld1) * sigma(Ld1,Ld2));
findalpha_sign0(P,Ld,Ld1,Ld2)..			alpha(P,Ld,Ld1,Ld2) =g= epsilon * alpha_plus(P,Ld,Ld1,Ld2) - V * (1 - alpha_plus(P,Ld,Ld1,Ld2));
findalpha_sign1(P,Ld,Ld1,Ld2)..			alpha(P,Ld,Ld1,Ld2) =l= V * alpha_plus(P,Ld,Ld1,Ld2);

*determine how much transcript J is deliberately transcribed from terminator
transcribed0(P,J,T,Ld,Ld1,Ld2)..		xi(P,J,T,Ld,Ld1,Ld2) =l= S(P) * M(P,J,T);
transcribed1(P,J,T,Ld,Ld1,Ld2)..		xi(P,J,T,Ld,Ld1,Ld2) =l= S(P) * alpha_plus(P,Ld,Ld1,Ld2);
transcribed2(P,J,T,Ld,Ld1,Ld2)..		xi(P,J,T,Ld,Ld1,Ld2) =g= S(P) * (M(P,J,T) + alpha_plus(P,Ld,Ld1,Ld2) - 1);

*make sure that enzyme is noted as being produced if it does have a non-zero concentration
produced0(E,Ld,Ld1,Ld2)..				omega(E,Ld,Ld1,Ld2) =l= V * C(E,Ld,Ld1,Ld2);
produced1(E,Ld,Ld1,Ld2)..				omega(E,Ld,Ld1,Ld2) =g= epsilon * C(E,Ld,Ld1,Ld2);

*determine the transcript level
trans_sum(J,T,Ld,Ld1,Ld2)..				phi(J,T,Ld,Ld1,Ld2) =e= sum(P, (xi(P,J,T,Ld,Ld1,Ld2) + M(P,J,T) * F(P)) * ((0.5) ** (1 / (G(T) + epsilon))));
...
prot_sum(E,Ld,Ld1,Ld2)..				C(E,Ld,Ld1,Ld2) =e= sum(J, (rho(J,E) * eta(J) * sum(T, phi(J,T,Ld,Ld1,Ld2))) * ((0.5) ** (1 / (R(E) + epsilon))));

*determine if concentration is at the level necessary for activity
act_threshold_0(E,Ld,Ld1,Ld2)..			(theta(E) + epsilon) * C_plus(E,Ld,Ld1,Ld2) =l= C(E,Ld,Ld1,Ld2);
act_threshold_1(E,Ld,Ld1,Ld2)..			C(E,Ld,Ld1,Ld2) =l= (V - (theta(E) - epsilon)) * C_plus(E,Ld,Ld1,Ld2) + (theta(E) - epsilon);

*get the value of gamma
findgamma(E,Ld,Ld1,Ld2)..				gamma(E,Ld,Ld1,Ld2) =e= Zeta(E) + sum(E1, W(E1,Ld,Ld1,Ld2) * B(E,E1) * Q(E,E1)) + B(E,Ld) * Q(E,Ld) + B(E,Ld1) * Q(E,Ld1) + B(E,Ld2) * Q(E,Ld2) - (B(E,Ld) * Q(E,Ld) * sigma(Ld,Ld1)) - (B(E,Ld) * Q(E,Ld) * sigma(Ld,Ld2)) - (B(E,Ld1) * Q(E,Ld1) * sigma(Ld1,Ld2));
findgamma_sign0(E,Ld,Ld1,Ld2)..			gamma(E,Ld,Ld1,Ld2) =g= epsilon * gamma_plus(E,Ld,Ld1,Ld2) - V * (1 - gamma_plus(E,Ld,Ld1,Ld2));
findgamma_sign1(E,Ld,Ld1,Ld2)..			gamma(E,Ld,Ld1,Ld2) =l= V * gamma_plus(E,Ld,Ld1,Ld2);

*determines if the protein is produced
prot_could_act0(E,Ld,Ld1,Ld2)..			kappa(E,Ld,Ld1,Ld2) =l= omega(E,Ld,Ld1,Ld2);
prot_could_act1(E,Ld,Ld1,Ld2)..			kappa(E,Ld,Ld1,Ld2) =l= gamma_plus(E,Ld,Ld1,Ld2);
prot_could_act2(E,Ld,Ld1,Ld2)..			kappa(E,Ld,Ld1,Ld2) =g= omega(E,Ld,Ld1,Ld2) + gamma_plus(E,Ld,Ld1,Ld2) - 1;

*if protein is both produced and is at the necessary concentration threshold for activation then it is active
prot_is_act0(E,Ld,Ld1,Ld2)..			W(E,Ld,Ld1,Ld2) =l= kappa(E,Ld,Ld1,Ld2);
prot_is_act1(E,Ld,Ld1,Ld2)..			W(E,Ld,Ld1,Ld2) =l= C_plus(E,Ld,Ld1,Ld2);
prot_is_act2(E,Ld,Ld1,Ld2)..			W(E,Ld,Ld1,Ld2) =g= kappa(E,Ld,Ld1,Ld2) + C_plus(E,Ld,Ld1,Ld2) - 1;

*need to constrain such that the logic table is satisfied
sat_lambda(Ld,Ld1,Ld2,Ed)..				W(Ed,Ld,Ld1,Ld2) =e= lambda(Ld,Ld1,Ld2,Ed);
...
explicit_1(E,Ld,Ld1,Ld2)..				W(E,Ld,Ld1,Ld2) =l= L(E);
explicit_2(E,Ld,Ld1,Ld2)..				kappa(E,Ld,Ld1,Ld2) =l= L(E);
explicit_3(E,Ld,Ld1,Ld2)..				omega(E,Ld,Ld1,Ld2) =l= L(E);
explicit_4(E,Ld,Ld1,Ld2)..				C(E,Ld,Ld1,Ld2) =l= V * L(E);
explicit_5(E,Ld,Ld1,Ld2)..				C_plus(E,Ld,Ld1,Ld2) =l= L(E);

*********************************************** EUGENECIM ********************************************************************

*objective function
obj_M..									Z_M =e= sum(Ed, sum(Ld, sum(Ld1, sum(Ld2, C(Ed,Ld,Ld1,Ld2)))));

*determine the transcript level
trans_level_M(J,T,Ld,Ld1,Ld2)..			phi(J,T,Ld,Ld1,Ld2) =e= sum(P, M_design(P,J,T) * alpha_plus(P,Ld,Ld1,Ld2) * S(P) + M_design(P,J,T) * F(P));

*determine protein concentration as a function of transcription and translation efficiency
*adds that produced from the previous time points RNA with that already present from previous time point
prot_conc_M(E,Ld,Ld1,Ld2)..				C(E,Ld,Ld1,Ld2) =e= sum(J, rho(J,E) * eta(J) * sum(T, RNATrans(J,T,Ld,Ld1,Ld2)));

*make sure that enzyme is noted as being produced if it does have a non-zero concentration
produced0_M(E,Ld,Ld1,Ld2)..				omega(E,Ld,Ld1,Ld2) =l= V * ECTrans(E,Ld,Ld1,Ld2);
produced1_M(E,Ld,Ld1,Ld2)..				omega(E,Ld,Ld1,Ld2) =g= epsilon * ECTrans(E,Ld,Ld1,Ld2);

*determine if concentration is at the level necessary for activity
act_threshold_0_M(E,Ld,Ld1,Ld2)..		(theta(E) + epsilon) * C_plus(E,Ld,Ld1,Ld2) =l= ECTrans(E,Ld,Ld1,Ld2);
act_threshold_1_M(E,Ld,Ld1,Ld2)..		ECTrans(E,Ld,Ld1,Ld2) =l= (V - (theta(E) - epsilon)) * C_plus(E,Ld,Ld1,Ld2) + (theta(E) - epsilon);
...
FILE SIMPLECID / a_simple_EUGENECID_report_test_adder.txt /;
PUT SIMPLECID;
SIMPLECID.pc = 2;
SIMPLECID.lw = 25;
...
FILE DETAILCID / a_detailed_EUGENECID_report_test_adder.txt /;
PUT DETAILCID;
DETAILCID.pc = 2;
DETAILCID.lw = 25;
...
FILE DESIGNS / circuit_designs_file_test_adder.txt /;
PUT DESIGNS;
DESIGNS.pc = 2;
DESIGNS.lw = 30;
...
FILE STATES / circuit_state_test_adder.csv /;
PUT STATES;
STATES.pc = 5;
STATES.lw = 30;
STATES.pw = 32767;
...
*make superheader
PUT "SOLN#","TIME","L1","L2","L3","PROMOTOR BEHAVIOR";
...
FILE TABLES / circuit_logic_tables_test_adder.txt /;
PUT TABLES;
TABLES.pc = 2;
TABLES.pw = 32767;
...
*write the header
PUT "Ligand 1",system.tab,"Ligand 2",system.tab,"Ligand 3",system.tab,system.tab,system.tab;
...
LOOP(Ld,
	...
	LOOP(Ld1,
		...
		LOOP(Ld2,
			...
			PUT Ld.tl,Ld1.tl,Ld2.tl;
			
			/*write the desired enzyme response to the system*/
			LOOP(Ed, 
			
				PUT lambda(Ld,Ld1,Ld2,Ed);
...
LOOP(big_set$(not done),
		...
		LOOP(Ld,
			...
			LOOP(Ld1,
				...
				LOOP(Ld2,
					...
					PUT "ligand condition: Ld: ",Ld.tl," Ld1: ",Ld1.tl," Ld2: ",Ld2.tl/;
					PUT "------------------------------------------------------------------------"//;
					...
					LOOP(j,
						...
						IF((sum(P, sum(T, M.l(P,J,T))) ge 1),
							...
							PUT j.tl,(sum(P, sum(T, xi.l(P,J,T,Ld,Ld1,Ld2)))),system.tab,sum(P, sum(T, M.l(P,j,T) * F(P))),system.tab,(sum(T, sum(P, xi.l(P,J,T,Ld,Ld1,Ld2) + M.l(P,J,T) * F(P)))),system.tab,((sum(T, sum(P, xi.l(P,J,T,Ld,Ld1,Ld2) + M.l(P,J,T) * F(P)))) - sum(T,sum(P, (xi.l(P,J,T,Ld,Ld1,Ld2) + M.l(P,J,T) * F(P)) * ((0.5) ** (1 / (G(T) + epsilon)))))),system.tab,(sum(T,phi.l(J,T,Ld,Ld1,Ld2)))/;
					...
					LOOP(E,
						...
						IF((C.l(E,Ld,Ld1,Ld2) > 0),
						
							PUT E.tl,C.l(E,Ld,Ld1,Ld2)/;
					...
					LOOP(E,
						...
						IF((C_plus.l(E,Ld,Ld1,Ld2) eq 1),
						
							PUT E.tl,C_plus.l(E,Ld,Ld1,Ld2)/;
					...
					LOOP(E,
						...
						IF((W.l(E,Ld,Ld1,Ld2) eq 1),
						
							PUT E.tl,W.l(E,Ld,Ld1,Ld2)/;
			...
			LOOP(Ld1,
				...
				LOOP(Ld2,
					...
					LOOP(P,
						...
						PUT P.tl,Z(P),(sum(E, (W.l(E,Ld,Ld1,Ld2) * I(P,E)))),I(P,Ld),I(P,Ld1),alpha.l(P,Ld,Ld1,Ld2),alpha_plus.l(P,Ld,Ld1,Ld2)/;
					...
					LOOP(J,
						...
						LOOP(T,
							...
							PUT J.tl,system.tab,T.tl,sum(P,xi.l(P,J,T,Ld,Ld1,Ld2)),(sum(P, M.l(P,J,T) * F(P))),(sum(P, xi.l(P,J,T,Ld,Ld1,Ld2) + M.l(P,J,T) * F(P)) * (1 - (0.5) ** (1 / (G(T) + epsilon)))),phi.l(J,T,Ld,Ld1,Ld2)/;
					...
					PUT "enzyme",system.tab,system.tab,system.tab,system.tab,"Zeta",system.tab,system.tab,"sum(W*B)",system.tab,"B(Ld)",system.tab,system.tab,"B(Ld1)",system.tab,system.tab,"B(Ld2)",system.tab,system.tab,"gamma",system.tab,system.tab,"gamma+",system.tab,system.tab,"omega",system.tab,system.tab,"kappa",system.tab,system.tab,"C",system.tab,system.tab,system.tab,"C+",system.tab,system.tab,system.tab,"W"/;
					
					LOOP(E,
					
						temp = sum(E1, W.l(E1,Ld,Ld1,Ld2) * B(E,E1));
						PUT E.tl,Zeta(E),temp,B(E,Ld),B(E,Ld1),B(E,Ld2),gamma.l(E,Ld,Ld1,Ld2),gamma_plus.l(E,Ld,Ld1,Ld2),omega.l(E,Ld,Ld1,Ld2),kappa.l(E,Ld,Ld1,Ld2),C.l(E,Ld,Ld1,Ld2),C_plus.l(E,Ld,Ld1,Ld2),W.l(E,Ld,Ld1,Ld2)/;
	...
	LOOP(tau$(not done_3),
		...
		IF(((EUGENECIM.modelStat eq 1) OR (EUGENECIM.modelStat eq 2) OR (EUGENECIM.modelStat eq 8) OR (EUGENECIM.modelStat eq 15) OR (EUGENECIM.modelStat eq 16) OR (EUGENECIM.modelStat eq 17)),
			...
			LOOP(Ld,
				...
				LOOP(Ld1,
					...
					LOOP(Ld2,
						...
						PUT big_set.tl,tau.tl,Ld.tl,Ld1.tl,Ld2.tl;
					
						/*write alpha values*/
						LOOP(P,
						
							PUT alpha.l(P,Ld,Ld1,Ld2);
						...
						LOOP(J,
							...
							PUT sum(T, phi.l(J,T,Ld,Ld1,Ld2));
						...
						LOOP(J,
							...
							PUT sum(T, RNATrans(J,T,Ld,Ld1,Ld2));
			...
			ECTrans(E,Ld,Ld1,Ld2) = (C.l(E,Ld,Ld1,Ld2) + ECTrans(E,Ld,Ld1,Ld2)) * (0.5 ** (1 / ((R(E)/2) + epsilon)));
				...
				LOOP(Ld,
					...
					LOOP(Ld1,
						...
						LOOP(Ld2,
							...
							IF((ECTrans(E,Ld,Ld1,Ld2) < 0),
							
								ECTrans(E,Ld,Ld1,Ld2) = 0;
			...
			RNATrans(J,T,Ld,Ld1,Ld2) = (phi.l(J,T,Ld,Ld1,Ld2) + RNATrans(J,T,Ld,Ld1,Ld2)) * (0.5 ** (1 / (sum(P, M_design(P,J,T) * (G(T)/2)) + epsilon))); 
					...
					LOOP(Ld,
						...
						LOOP(Ld1,
							...
							LOOP(Ld2,
								...
								/*if less than zero reset to zero*/
								IF((RNATrans(J,T,Ld,Ld1,Ld2) < 0),
								
									RNATrans(J,T,Ld,Ld1,Ld2) = 0;
